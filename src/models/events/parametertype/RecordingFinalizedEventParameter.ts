...
export interface IRecordingFinalizedEventParameter  {
    callId : string;
    finalRecordingReference : string;
    fileName : string;
    eventTime : string;
    hubEventTime? : string;
}
...
export class RecordingFinalizedEventParameter {
    ...
    */
    ...
    /**
    * final URI to download recording
    */
    private  _finalRecordingReference : string;
    ...
    */
    ...
    constructor (params :IRecordingFinalizedEventParameter) {
        ValidationManager.validateAttributes(params, ['callId', 'finalRecordingReference', 'fileName', 'eventTime'], ['callId', 'finalRecordingReference', 'fileName', 'eventTime', 'hubEventTime']);
        ...
        if ( !/^https?:\/\/[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9:_\+\/=]*).wav\?token=[a-f0-9]+$/.test(params.finalRecordingReference)) {
            const error = ErrorManager.exception({
                error_code: TTErrorCodes.TTLEC3024_FIELD_PATTERN_MISMATCH_ERROR,
                data: {
                    $field: 'finalRecordingReference',
                    "$pattern": '^https?:\/\/[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9:_\+\/=]*).wav\?token=[a-f0-9]+$',
                }
            });
            Error.captureStackTrace(error);
            logger.error(error.message);
        ...
        this._finalRecordingReference = params.finalRecordingReference;
    ...
    get finalRecordingReference(): string  {
        return this._finalRecordingReference;
    }

    set finalRecordingReference(finalRecordingReference: string) {
        if ( !/^https?:\/\/[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9:_\+\/=]*).wav\?token=[a-f0-9]+$/.test(finalRecordingReference)) {
            const error = ErrorManager.exception({
                error_code: TTErrorCodes.TTLEC3024_FIELD_PATTERN_MISMATCH_ERROR,
                data: {
                    finalRecordingReference: 'finalRecordingReference',
                    "$pattern": '^https?:\/\/[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9:_\+\/=]*).wav\?token=[a-f0-9]+$',
                }
            });
            Error.captureStackTrace(error);
        ...
        this._finalRecordingReference = finalRecordingReference;
    }
    ...
    toJSON():  IRecordingFinalizedEventParameter  {
        return {
            callId: this._callId,
            finalRecordingReference: this._finalRecordingReference,
...
class Builder {
    ...
    finalRecordingReference(finalRecordingReference: string) : this{
        this._finalRecordingReference = finalRecordingReference;
        return this;
    }
    ...
    build() : RecordingFinalizedEventParameter {
        return new RecordingFinalizedEventParameter({
